    ...
    async function runTvAdaptationSelfTests() {
      ...
      const sidebarNavigationSource = String(performSidebarNavigation);
      const sidebarRequestSource = String(requestSidebarNavigation);
      ...
      check("PHASE3_SIDEBAR_REAL_NAVIGATION", typeof performSidebarNavigation === "function" && sidebarNavigationSource.includes("openSecondaryCatalog") && sidebarNavigationSource.includes("openLiveHome") && sidebarNavigationSource.includes("openSettingHome") && sidebarRequestSource.includes("performSidebarNavigation"), "Sidebar delegates navigation through performSidebarNavigation", "STATIC_HOOK");
      ...
      const animeFailureDStoreSizeAfterFailure = Object.keys(animeFailureD.store).length;
      ...
      check("NOSTR_ANIME_CASE_D_FAILURE_RETRY", animeFailureD.runtime.metrics.newDetailRequests === 1 && animeFailureDStoreSizeAfterFailure === 0 && animeFailureDSecond.runtime.metrics.newDetailRequests === 1 && Object.keys(animeFailureD.store).length === 1 && animeFailureDSecond.items.length === 1, "failed detail is not negative-cached, retries, and caches the later success", "RUNTIME_MOCK");
      ...
      check("NOSTR_ANIME_PERSISTENT_CACHE_LAYER", animeResolverSource.includes("ensureNostrTmdbMetaLoaded") && animeResolverSource.includes("nostrTmdbRecordFromDetail") && String(nostrTmdbRecordFromDetail).includes("nostrTmdbMetaPut") && animeCacheSource.includes("NOSTR_TMDB_META_CACHE_TTL_MS"), "anime resolver uses one persistent TTL cache", "STATIC_HOOK");
      check("NOSTR_ANIME_FAILURE_NOT_CACHED", animeResolverSource.includes('record.status !== "valid"') && animeResolverSource.includes('record.status !== "invalid"') && animeResolverSource.includes("delete memory[key]"), "only valid or invalid detail records remain cached; other results stay retryable", "STATIC_HOOK");
      ...
      check("HOME_BADGE_VISUAL_CONSISTENCY", cardRendererSource.includes('class="badge-icon rating-icon"') && cardRendererSource.includes('class="badge-icon people-icon"') && metadataContractCss.includes("font-weight: 760 !important") && metadataContractCss.includes("padding: 0 !important") && metadataContractCss.includes("gap: 3px !important"), "Home badge markup and CSS share the frameless visual language", "STATIC_HOOK");
      ...
      const manualUnreleasedGuardAt = detailRequestSource.indexOf('settleDetailUnreleased(item, "unreleased_manual_guard")');
      const manualUnreleasedGuardEndAt = manualUnreleasedGuardAt >= 0
        ? detailRequestSource.indexOf("return;", manualUnreleasedGuardAt) + "return;".length
        : -1;
      const manualUnreleasedGuardBlock = manualUnreleasedGuardAt >= 0 && manualUnreleasedGuardEndAt > manualUnreleasedGuardAt
        ? detailRequestSource.slice(manualUnreleasedGuardAt, manualUnreleasedGuardEndAt)
        : "";
      ...
      check("UNRELEASED_MANUAL_PLAY_BLOCKED", requestUnreleasedAt >= 0 && manualUnreleasedGuardAt > requestUnreleasedAt && manualUnreleasedGuardAt < requestDirectAt && manualUnreleasedGuardBlock.includes('toast("尚未上映")') && manualUnreleasedGuardBlock.includes("return;"), "manual Play remains focusable but the explicit unreleased guard returns before Direct playback", "STATIC_HOOK");
      check("UNRELEASED_MANUAL_CLICK_BLOCKED", requestUnreleasedAt >= 0 && requestUnreleasedAt < manualUnreleasedGuardAt && manualUnreleasedGuardAt < requestDirectAt && manualUnreleasedGuardBlock.includes('settleDetailUnreleased(item, "unreleased_manual_guard")'), "manual Play has a second unreleased guard", "STATIC_HOOK");
      ...
      check("DETAIL_RACE_TENTATIVE_HOLD", raceTentativeHold && String(detailRaceLaneTerminal).includes("allowTentativeFinal") && String(tryCommitDetailProvider).includes("allowTentativeFinal") && String(tryCommitDetailProvider).includes("run.curatedTerminal") && String(tryCommitDetailProvider).includes("run.directTerminal"), "tentative candidate waits for the other lane and is only final fallback", "RUNTIME_MOCK");
      ...
      check("CURATED_LATE_HEALTH_DEADLINE_HARD_LIMIT", lateHealthCollectionSource.includes('finishCollection("DISCOVERY_DEADLINE")') && lateHealthCollectionSource.includes("Date.now() >= discoveryDeadline") && lateHealthCollectionSource.includes('ignoredReason = "DISCOVERY_DEADLINE"') && lateHealthCollectionSource.includes("reason: ignoredReason"), "the existing discovery deadline remains the hard upper bound", "STATIC_HOOK");
      ...
      const curatedProgressiveHealthSource = String(curatedHealthBatchPlan) + String(curatedProgressiveHealthCandidate);
      check("CURATED_PROGRESSIVE_HEALTH", curatedProgressiveHealthSource.includes("CURATED_HEALTH_BATCH") && curatedProgressiveHealthSource.includes("CURATED_MAX_AUTO_CHECK_PER_SOURCE") && curatedProgressiveHealthSource.includes("CURATED_PREFLIGHT_PER_SOURCE"), "Curated qualification is bounded and progressive", "STATIC_HOOK");
      ...
      check("PHASE3_HERO_CONTENT_OFFSET", document.documentElement.innerHTML.includes("top: calc(clamp(220px, 13vw, 270px) + 108px)"), "Hero foreground uses the current visual baseline offset", "STATIC_HOOK");
      check("PHASE3_HERO_DOTS_UNCHANGED", document.documentElement.innerHTML.includes("bottom: clamp(54px, 5.2vw, 78px)"), "Hero dots use the current visual baseline", "STATIC_HOOK");
